...
const REFERENCE_PAGES = {
  ...
  metodologia: {
    ...
    color: '#8B5CF6',
    phases: [
      { num: -1, name: 'Backlog',         gate: 'Proyecto creado desde la OV. Sin equipo asignado todavia.', duration: 'variable',     color: '#94A3B8' },
  ...
  'catalogo-torus': {
    ...
    projectStandards: {
      ...
      projectStages: {
        title: 'Stages de Proyecto (project.project.stage)',
        alert: 'Pipeline lineal compartido por todos los proyectos cliente. Backlog antes de Kick-off, terminales fold=true.',
        list: [
          { id: 6,  seq: 0,  name: 'Templates',       fold: true,  desc: 'Solo project templates oficiales. NO mover proyectos cliente aqui.' },
          { id: 5,  seq: 1,  name: 'Internos',        fold: false, desc: 'Proyectos internos del equipo (sin venta asociada).' },
          { id: 23, seq: 2,  name: 'Backlog',         fold: false, desc: 'Vendido pero no arrancado. Sin equipo asignado todavia. Aqui no se valida enforcement.' },
          { id: 14, seq: 3,  name: '0 · Kick-off',    fold: false, desc: 'Reunion oficial con cliente. Acta firmada → pasa a Analisis.' },
          { id: 15, seq: 4,  name: '1 · Análisis',    fold: false, desc: 'Relevamiento + GAP analysis. Acta firmada → pasa a Implementacion.' },
          { id: 16, seq: 5,  name: '2 · Implementación', fold: false, desc: 'Configuracion + customizaciones por sprints.' },
          { id: 17, seq: 6,  name: '3 · Capacitación', fold: false, desc: 'Entrenamiento a usuarios del cliente.' },
          { id: 18, seq: 7,  name: '4 · UAT',         fold: false, desc: 'Cliente prueba en staging. Acta UAT → autoriza cut-over.' },
          { id: 19, seq: 8,  name: '5 · Cut-over',    fold: false, desc: 'Migracion final + switch al sistema productivo.' },
          { id: 20, seq: 9,  name: '6 · Go-Live',     fold: false, desc: 'Cliente opera en produccion. Acta de Go-Live → inicia garantia.' },
          { id: 21, seq: 10, name: '7 · Estabilización', fold: false, desc: '30 dias de soporte garantia post Go-Live.' },
          { id: 22, seq: 11, name: '8 · Cerrado',     fold: true,  desc: 'Stage terminal. Acta de cierre firmada.' },
          { id: 4,  seq: 13, name: 'Cancelada',       fold: true,  desc: 'Proyecto cancelado antes de Go-Live.' },
        ],
        rules: [
          'Backlog es el stage default cuando se confirma una OV. El proyecto queda ahi sin equipo hasta que el PM agende kick-off.',
          'Mover de Backlog → Kick-off implica que ya esta agendada la reunion con cliente.',
          'NO mover proyectos cliente a Templates (ese stage es solo para los project templates oficiales).',
          'Proyectos sin OV vinculada van a Internos, no al flujo Kick-off → Cerrado.',
        ],
      },
      autoNaming: {
        title: 'Auto-naming de Proyectos desde OV',
        alert: 'Al confirmar una OV, el proyecto se nombra automaticamente. NO renombrar a mano salvo casos legacy.',
        format: 'SXXXXX - [TIPO] - COMMERCIAL_PARTNER',
        examples: [
          'S00089 - [CORE] - DYD Arquitectura y Construccion SRL',
          'S00047 - [HOURS] - PALKE S.A.',
          'S00097 - [HOURS] - DDS SOCIEDAD DE RESPONSABILIDAD LIMITADA',
        ],
        rules: [
          'SXXXXX = numero de la sale.order (campo name)',
          '[TIPO] = product.product.default_code (envuelto en corchetes por la automation)',
          'COMMERCIAL_PARTNER = sale.order.commercial_partner_id.name (la EMPRESA, no el contacto persona)',
          'Si el partner del SO es un contacto (is_company=False) bajo una empresa, Clockify y el naming usan la empresa parent.',
          'Renaming dispara sync automatico a Clockify (modulo clockify_integration). No tocar Clockify a mano.',
        ],
        implementation: 'base.automation id=51 con trigger on_create_or_write sobre sale_line_id. Llama ir.actions.server con codigo Python que escribe el nombre en cada idioma (es_PY + en_US, gotcha translatable).',
      },
      serviceTypeField: {
        title: 'Campo x_service_type (Tipo de Servicio)',
        alert: 'Reemplaza el patron viejo de tags-de-servicio en project.tags. tag_ids ahora es exclusivo de tareas.',
        description: 'Char editable en project.project que clasifica el proyecto por tipo de servicio ([CORE], [HOURS], [SUPP], etc.). Visible en el card kanban (badge gris) y en el form (campo readonly). Auto-poblado desde sale_line_id.product_id.default_code via base.automation.',
        rules: [
          'Cuando cambia el producto de la OV → la automation pisa x_service_type al nuevo default_code (con corchetes)',
          'Si el producto no tiene default_code → NO toca (preserva override manual)',
          'Sin sale_line_id vinculada → editable manual libre',
          'NO usar tag_ids para clasificar proyectos por servicio. Tags = solo para tareas.',
        ],
        implementation: 'base.automation id=52 con trigger on_create_or_write sobre sale_line_id. Field id 23408 (state=manual, store=True, index=True). Vista kanban id 5583 con badge, vista form id 5584 con campo editable.',
      },
      enforcement: {
        title: 'Enforcement de Campos Obligatorios al Cambiar Etapa',
        alert: 'Al mover un proyecto a una etapa de trabajo, si faltan campos obligatorios, Odoo tira error y bloquea el cambio.',
        triggerStages: 'Aplica al pasar a 0·Kick-off, 1·Analisis, 2·Implementacion, 3·Capacitacion, 4·UAT, 5·Cut-over, 6·Go-Live, 7·Estabilizacion, 8·Cerrado.',
        exemptStages: 'NO aplica si destino es Backlog, Cancelada, Internos o Templates.',
        requiredAlways: [
          'partner_id (Cliente)',
          'user_id (Gerente de Proyecto)',
          'sale_order_id (Orden de venta)',
          'sale_line_id (Articulo de la orden de venta)',
          'x_service_type (Tipo de servicio)',
        ],
        requiredConditional: [
          { field: 'date_start + date',        when: 'Obligatorio EXCEPTO si x_service_type IN [HOURS], [HOURLY], [SUPP]' },
          { field: 'allocated_hours > 0',     when: 'Obligatorio EXCEPTO si x_service_type == [HOURLY]' },
          { field: 'scrum_team_id',           when: 'Obligatorio EXCEPTO si destino es 0·Kick-off (el equipo se asigna despues)' },
        ],
        userErrorTemplate: "No se puede mover el proyecto 'XXXXX' a la etapa 'YYY'. Faltan los siguientes campos obligatorios: - Cliente, - PM, - Orden de venta, ...",
        rules: [
          'Si vas a empezar a trabajar un proyecto y te falta un campo, cargalo en Backlog antes de pasarlo a Kick-off.',
          'En Kick-off no se exige scrum_team — se asigna durante o despues de la reunion oficial.',
          'Tipos sin fechas/horas (HOURS/HOURLY/SUPP) son recurrentes — no aplica el control de fechas.',
          'Para proyectos legacy que no se van a tocar mas, archivar (active=False) en lugar de moverlos de etapa.',
        ],
        implementation: 'base.automation id=53 con trigger on_create_or_write sobre stage_id. ir.actions.server id=1566 con codigo Python que arma lista de campos faltantes y hace raise UserError. Live desde 2026-04-30.',
      },
    },
  },
};
